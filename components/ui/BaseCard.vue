<template>
  <keep-alive>
    <div v-if="windowWidth > 0" class="movie-card" :style="cardSize()">
      <div class="movie-img" @mouseover="viewOverview(true)" @mouseout="viewOverview(false)">
        <h2 class="review">
          {{movie.vote_average}}
        </h2>
      ...
      <nuxt-link :id="movie.id" :to="{name: 'movies-movieid', params: {movieid: movie.id}}">Get More Info</nuxt-link>
      <div class="release">Lançado em:<br>
        {{new Date(movie.release_date).toLocaleString('pt-br', {month:'long', day: 'numeric', year: 'numeric'})}}
      </div>
    </div>
  </keep-alive>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      responsiveImg(movie) {
        return `https://image.tmdb.org/t/p/w${this.getCardWidth()}/${movie.poster_path}`
      },
      cardSize() {
